...
class MarvelCharacterBootstrapTest extends Specification {
    ...
    void setup() {
        ...
        marvelCharacterBootstrap.setRestTemplate(restTemplate)
    }

    def "shouldLoadAllCharactersFromMarvel"() {
        given:
        def marvelJsonReplyWith3Characters = TestHelper.ReadFileFromResources("MarvelCharactersResponse.json")
        when:
        marvelCharacterBootstrap.onApplicationEvent(null)
        then:
        1 * marvelConfig.setTimestamp(_);
        ...
        savedListCharacters.get(0).wikiUrl == "http://marvel.com/universe/3-D_Man_(Chandler)?utm_campaign=apiRef&utm_source=debb99fafc067d709ed11e9e2014343d"
        ...
        savedListCharacters.get(1).wikiUrl == null
        ...
        savedListCharacters.get(2).wikiUrl == "http://marvel.com/universe/A.I.M.?utm_campaign=apiRef&utm_source=debb99fafc067d709ed11e9e2014343d"
    }

    def "shouldLoadCharactersUntilNoMore"() {
        given:
        def marvelJsonReplyWith1Character =
                TestHelper.ReadFileFromResources("MarvelCharactersResponseCount1Total3.json")
        when:
        marvelCharacterBootstrap.onApplicationEvent(null)
        then:
        1 * marvelConfig.getCharactersUrl(0) >> charactersUrl
    ...
    def "shouldThowIOException"() {
        given:
        def marvelJsonReplyWith1Character = "{\"data\":}";
        when:
        marvelCharacterBootstrap.onApplicationEvent(null)
        then:
        1 * marvelConfig.getCharactersUrl(0) >> charactersUrl
        1 * restTemplate.getForObject(charactersUrl, String.class) >> marvelJsonReplyWith1Character
    }
}
